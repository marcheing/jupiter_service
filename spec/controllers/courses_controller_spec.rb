...
describe CoursesController do
  describe 'course' do
    context 'MAC0101 2016 2nd semester' do
    ...
    context 'CBM0130 2016 2nd semester' do
      let(:course_code) { 'CBM0130' }

      it 'correctly parses the course without professor information' do
        VCR.use_cassette "courses/#{course_code}" do
          get :course, params: { code: course_code }

          response_body = JSON.parse response.body
          expect(response_body['course']['professors']).to be_empty
        end
      end
    end
  end
